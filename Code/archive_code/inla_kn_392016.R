...
mlm.stack.est <- inla.stack(data =list(y=y), 
                            ...
                            tag="est" )
output2.mlm <- inla (formula2.mlm,  data=inla.stack.data(mlm.stack.est, spde=spde), control.predictor =list(A=inla.stack.A(mlm.stack.est), compute=TRUE))
...
data = list(y=y, GW_dnsty15= dsnew$GW_dnsty15, WR_density=dsnew$WR_density, delta_lc=dsnew$delta_lc, HUCID=dsnew$HUCID, spatial.field=1:spde$n.spde) #HUCID not found, if remove control.predictor then intercept is not found
output.mlm <- inla (formula.mlm,  data=data, control.predictor =list(A=A.est, compute=TRUE))
mlm.summary <- summary (output.mlm)