...
irr_wr <-read_excel("irrigation.xls", sheet = "Water Rights", col_names=TRUE)
irr_pod <-read.csv("irr_pod.csv") #some problems with this sheet, export to a csv and read in seperately
irr_app <- read_excel("irrigation.xls", sheet = "Application Info", col_names=TRUE, 
                      col_types = c("text", "text", "text", "text", "text", "text", "text", "text", "text", "text", "text", "text", "numeric", "text"))
irr_use <- read.csv("irr_uses.csv") #some problems with this sheet, export to a csv and read in seperately

irr_wr <- left_join (irr_wr, irr_use, by = c("Application Number" = "Application.Number")) #join use info to wr
irr_wrapp <- left_join(irr_wr, irr_app, by = c("Application Number" = "Application ID")) #join app info to wr
irr_podwrapp <- left_join(irr_pod, irr_wrapp, by = c("Application.Number" = "Application Number")) #join wr to pods
write.csv(irr_podwrapp, "irr_podwrapp.csv") #write the full dataset for eac use to its own csv
...
pow_podwrapp$POD.Num <- as.character(pow_podwrapp$POD.Num)
...
incp_podwrapp$POD.Num <- as.character(incp_podwrapp$POD.Num)
...
mil_podwrapp$POD.Num <- as.character(mil_podwrapp$POD.Num)
...
snow_podwrapp$POD.Num <- as.character(snow_podwrapp$POD.Num)
...
aes_podwrapp$POD.Num <- as.character(aes_podwrapp$POD.Num)
...
###JOIN records into simplified use Categories####
#conduct joins in order of relative importance (subjective decision) so that uses are ordered
...
ag_dat <- ag[,(names(ag) %in% keep)]
ag_dat$Latitude <- as.numeric(ag_dat$Latitude, options(digits = 15))
ag_dat$Longitude <- as.numeric(ag_dat$Longitude, options(digits = 15))
ag_dat$'Status Date' <- as.Date(ag_dat$'Status Date', "%m/%d/%Y")
ag_dat <- filter(ag_dat, !is.na(Latitude))
ag_dat <- distinct(ag_dat) #keeps the first instance
...
###Put them all together then pull out by year####
full.pt<-rbind(ag_data,dom_data,ind_data,wild_data,other_data)

# keep only  water rights which had a status change before or during the year of interest
# this keeps out wrs that were activated during a later year 
...
# this should leave only wrs which were active for all or some part of the year of interest
...
###Write wr point data to ESRI shapefiles ###
