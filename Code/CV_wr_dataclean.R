...
#data extracted for entire state extent by individual use category
#################################################################

##read in full irrigation water rights data
...
#join irrigation water rights information 
...
##read in full domestic water rights data
...
#join domestic water rights information
...
##read in full industrial water right data
...
#join industrial water right information
...
write.csv(ag, "agriculture_wr.csv") #agriculture uses

dom <- full_join(dom_podwrapp,mun_podwrapp)
dom <- full_join(dom,aes_podwrapp)
write.csv(dom, "domestic_wr.csv") #domestic uses
...
write.csv(ind, "industrial_wr.csv") #industrial uses

wild <- full_join(wild_podwrapp, fire_podwrapp)
write.csv(wild, "wildlife_wr.csv") #fish & wildlife uses

other <- full_join(rec_podwrapp,oth_podwrapp)
other <- full_join(other,snow_podwrapp)
write.csv(other, "other_wr.csv")  #recreational uses
...
#this process helps to remove duplicate records for a single water right
#point of distribution with multiple records associated with supplemental 
